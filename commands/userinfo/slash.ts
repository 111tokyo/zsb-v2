import { ChatInputCommandInteraction, Role, roleMention, SlashCommandBuilder, time } from 'discord.js';
...
export const slashCommand: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .setDescriptionLocalization('fr', 'Obtenir des informations sur un utilisateur')
    .addUserOption(option =>
      option
        .setName('user')
        .setDescription('The user to get information about')
        .setDescriptionLocalization('fr', 'L\'utilisateur dont vous voulez voir les informations')
        .setRequired(false)),
  ...
  execute: async (selfbotUser: SelfbotUser, interaction: ChatInputCommandInteraction) => {
    ...
    if (interaction.guildId) {
      guild = await selfbotUser.guilds.fetch(interaction.guildId).catch(() => null);
      member = (guild ? await guild.members.fetch(targetUser.id).catch(() => null) : null);
    }
    if (!member) {
    ...
    const roles = member ? member.roles.cache.map((r: Role) => roleMention(r.id)).join(", ") : "`N/A`";
    const userBanner = await targetUser.fetch().then(u => u.bannerURL({ size: 1024 }));
    ...
    const badges =
      flags.length > 0
        ? flags
          .map((flag: string) => {
            switch (flag) {
              case "Staff": return "`Discord Employee`";
              ...
              case "VerifiedDeveloper": return "`Early Verified Bot Developer`";
              case "CertifiedModerator": return "`Discord Certified Moderator`";
              case "BotHTTPInteractions": return "`HTTP Interactions Bot`";
            ...
            components: [
              {
                ...
                content: [
                  ...
                  `**${selfbotUser.lang === 'fr' ? 'Nitro' : 'Nitro'}:** **${mutuals.premium_type === 3 ? '`Nitro Basic`' :
                    mutuals.premium_type === 2 ? '`Nitro Boost`' : '`❌`'
                  }**`,
            ...
            components: [
              {
                type: 2,
                style: 5,
                label: selfbotUser.lang === 'fr' ? "URL de l'avatar" : "Avatar URL",
                emoji: null,
                disabled: false,
                url: targetUser.displayAvatarURL({ size: 4096 })
              },
              ...(userBanner ? [{
                type: 2,
                style: 5,
                label: selfbotUser.lang === 'fr' ? "URL de la bannière" : "Banner URL",
                emoji: null,
                disabled: false,
                url: userBanner
              }] : [])
