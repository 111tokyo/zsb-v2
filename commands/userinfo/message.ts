...
export const messageCommand: MessageCommand = {
  async execute(selfbot, selfbotUser, message, args: string[]) {
    ...
    let targetUser = selfbot.users.cache.get(targetId);

    if (!targetUser) {
      targetUser = await selfbot.users.fetch(targetId)
    }

    if (!targetUser) {
      await message.edit(selfbotUser.lang === 'fr'
    ...
    const roles = member ? member.roles.cache.map(r => r.name).join(", ") : "`N/A`";
          ...
          "components": [
            ...
            ...(message.guild ? [{
              ...
              "accessory": {
                "type": 11,
                "media": {
                  "url": message.guild.iconURL({ size: 4096 }) || message.guild.splashURL({ size: 4096 }) || message.guild.bannerURL({ size: 4096 }) || message.guild.discoverySplashURL({ size: 4096 }),
                },
                "description": null,
                "spoiler": false
              },
