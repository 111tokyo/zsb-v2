  ...
  async execute(_selfbot, selfbotUser, message, args: string[]) {
    ...
    if(!member?.permissions.has(PermissionsBitField.Flags.BanMembers)) {
      await message.edit({
        content:
          selfbotUser.lang === 'fr'
            ? `**Vous n'avez pas les permissions pour bannir un utilisateur!**\n-# ➜ *Suppression du message ${time(now + 16, 'R')}*`
            : `**You do not have permission to ban a user!**\n-# ➜ *Deleting message ${time(now + 16, 'R')}*`,
      });
      return;
    }
    const user = await guild?.members.fetch(args[0]).catch(async () => {
      ...
      await message.edit({
        content:
          selfbotUser.lang === 'fr'
            ? `**Vous ne pouvez pas bannir un utilisateur qui a une rôle plus haut ou égal à votre rôle!**\n-# ➜ *Suppression du message ${time(now + 16, 'R')}*`
            : `**You cannot ban a user who has a role higher or equal to your role!**\n-# ➜ *Deleting message ${time(now + 16, 'R')}*`,
      });
      return;
    }
    ...
    if(member.permissions.has(PermissionsBitField.Flags.BanMembers)) {
        ...
        await message.edit({
            content:
              selfbotUser.lang === 'fr'
                ? `**L'utilisateur ${user} a été banni avec succès!**\n-# ➜ *Suppression du message ${time(now + 16, 'R')}*`
                : `**The user ${user} has been banned successfully!**\n-# ➜ *Deleting message ${time(now + 16, 'R')}*`,
          });
          return;
    } 
    return;
