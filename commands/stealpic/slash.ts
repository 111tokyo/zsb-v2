  ...
  execute: async (
    ...
    if (ok) {
      await selfbotUser.user
        ?.setAvatar(targetAvatar!.avatarURL({ format: 'png' }))
        .catch(async () => {
      ...
      await selfbotUser.user
        ?.setAvatar(targetAvatar!.avatarURL({ dynamic: true }))
        .catch(async () => {
          await interaction.reply({
            content:
              selfbotUser.lang === 'fr'
    ...
    await interaction.reply({
      content:
        selfbotUser.lang === 'fr'
          ? `Vous avez volé l'avatar de ${user}${ok ? ", malheuresement sa photo est un gif donc je l'ai converti en png." : '.'}`
          : `You stole ${user}'s avatar${ok ? ', unfortunately his photo is a gif so I converted it to png.' : '.'}`,
      flags: MessageFlags.Ephemeral,
    });
    return;
  },
